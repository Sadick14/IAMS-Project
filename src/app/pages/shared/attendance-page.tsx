...
const STATUS_COLORS: Record<string, string> = {
  present: "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-950/30 dark:text-emerald-400 dark:border-emerald-800",
  late: "bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-950/30 dark:text-amber-400 dark:border-amber-800",
  half_day: "bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-950/30 dark:text-blue-400 dark:border-blue-800",
  excused: "bg-violet-50 text-violet-700 border-violet-200 dark:bg-violet-950/30 dark:text-violet-400 dark:border-violet-800",
  absent: "bg-red-50 text-red-700 border-red-200 dark:bg-red-950/30 dark:text-red-400 dark:border-red-800",
};
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setSelectedRecord(null)}>
          <div className="bg-card border border-border rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
              ...
              <div>
                ...
                {selectedRecord.verified_by ? (
                  <div className="flex flex-col gap-1">
                    <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-100 text-emerald-700 border border-emerald-200 w-fit" style={{ fontSize: "0.8rem" }}>
                      <CheckCircle2 className="w-3.5 h-3.5" /> Verified
                    </span>
                    <p className="text-muted-foreground text-[0.7rem]">
                      Verified on {new Date(selectedRecord.updated_at || selectedRecord.attendance_date).toLocaleDateString()}
                    </p>
                  </div>
                ) : (
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-amber-100 text-amber-700 border border-amber-200 w-fit" style={{ fontSize: "0.8rem" }}>
            ...
            {canVerify && !selectedRecord.verified_by && (
              <div className="px-6 py-4 border-t border-border bg-secondary/20 flex flex-col sm:flex-row justify-end gap-2">
                <button
                  onClick={() => handleVerify(String(selectedRecord.id), "present")}
                  className="flex items-center justify-center gap-1.5 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors"
                  style={{ fontSize: "0.85rem" }}
                >
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-1.5 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors"
                  style={{ fontSize: "0.85rem" }}
                >
                  <XCircle className="w-4 h-4" /> Reject (Mark Absent)
                </button>
              </div>
            )}
          </div>
                ...
                filtered.map((r: any) => (
                  <tr key={r.id} className="border-b border-border last:border-0 hover:bg-secondary/25 cursor-pointer" onClick={() => setSelectedRecord(r)}>
                    <td className="px-4 py-4">
                      <p style={{ fontSize: "0.85rem" }} className="font-medium">{studentName(r)}</p>
                    ...
                    <td className="px-4 py-4">
                      <div className="flex flex-col gap-0.5">
                        ...
                        <span className="text-muted-foreground" style={{ fontSize: "0.7rem" }}>
                          {r.check_out_time ? `Out: ${r.check_out_time}` : "Auto-out"}
                        </span>
                      </div>
                    </td>
                    <td className="px-4 py-4">
                      {r.gps_check_in_lat ? (
                        <div className="flex items-center gap-1 text-emerald-600" title="GPS Verified">
                          <Navigation className="w-3 h-3" />
                          <span style={{ fontSize: "0.75rem" }}>GPS</span>
                        </div>
                      ) : (
                        <div className="flex items-center gap-1 text-muted-foreground" title="Manual Entry">
                          <MapPin className="w-3 h-3" />
                          <span style={{ fontSize: "0.75rem" }}>Manual</span>
                        </div>
                      )}
                    </td>
                    <td className="px-4 py-4">
                      <span className={`px-2 py-1 rounded-lg capitalize border ${STATUS_COLORS[r.status] ?? "bg-gray-100 text-gray-600 border-gray-200"}`} style={{ fontSize: "0.7rem", fontWeight: 500 }}>
                        {r.status}
                      </span>
                    </td>
                    <td className="px-4 py-4">
                      {r.verified_by
                        ? <div className="flex items-center gap-1 text-emerald-600" style={{ fontSize: "0.75rem" }}><CheckCircle2 className="w-4 h-4" /> <span>Verified</span></div>
                        : <span className="flex items-center gap-1 text-amber-600" style={{ fontSize: "0.75rem" }}><Clock className="w-3.5 h-3.5" /> <span>Pending</span></span>}
                    </td>
                  </tr>
                ))
              )}
